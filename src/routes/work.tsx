  ...
  return (
    <div className="grain mx-auto max-w-7xl px-4 sm:px-6 py-16">
      <div className="max-w-3xl">
        <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground">/ The work</p>
        ...
        <p className="mt-5 text-lg text-muted-foreground">
          Six polished landing pages built for different brands and products, plus a separate set of miscellaneous projects that show the full range of my web work.
        </p>
      </div>
      ...
      <div className="mt-20 max-w-3xl">
        <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground">/ Miscellaneous projects</p>
        <h2 className="mt-2 font-display text-4xl sm:text-5xl">Builds beyond landing pages.</h2>
      </div>

      <div className="mt-8 grid gap-6 md:grid-cols-3">
        {MISC_PROJECTS.map((project) => (
          <article key={project.title} className="bg-card brutal rounded-2xl p-6 border-2 border-ink">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="font-mono text-xs uppercase tracking-widest text-muted-foreground">{project.where}</p>
                <h3 className="mt-2 font-display text-3xl">{project.title}</h3>
              </div>
              <span className="font-mono text-xs px-2 py-1 bg-lime rounded-md brutal-sm">{project.year}</span>
            </div>
            <p className="mt-4 text-sm text-muted-foreground leading-relaxed">{project.blurb}</p>
            <div className="mt-5 flex flex-wrap gap-1.5">
              {project.stack.map((stackItem) => (
                <span key={stackItem} className="font-mono text-[10px] px-2 py-0.5 bg-muted border border-ink/20 rounded">
                  {stackItem}
                </span>
              ))}
            </div>
