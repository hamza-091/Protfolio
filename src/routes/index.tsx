    ...
    <div className="grain" ref={revealRef}>
      ...
      <section className="relative overflow-hidden border-b-2 border-ink">
        <div className="absolute inset-0 -z-10">
          ...
          <div
            className="absolute top-36 -right-8 w-36 h-36 sm:w-64 sm:h-64 lg:-right-24 lg:w-95 lg:h-95 bg-pink blob opacity-60"
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 pt-14 pb-20 grid lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-7">
            ...
            <p className="mt-6 max-w-xl text-lg text-muted-foreground">
              I build sharp, scroll-stopping websites with clean structure, modern motion, and a
              little edge.
            </p>
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                to="/work"
                className="inline-flex items-center gap-2 px-5 py-3 bg-ink text-cream font-mono text-sm rounded-md brutal brutal-hover"
              >
                View work <ArrowRight className="w-4 h-4" />
              </Link>
              <Link
                to="/contact"
                className="inline-flex items-center gap-2 px-5 py-3 bg-cream font-mono text-sm rounded-md brutal brutal-hover"
              >
                Get in touch
              </Link>
            </div>
            ...
            <div className="mt-6 inline-flex items-center gap-2 px-3 py-1.5 bg-card brutal-sm rounded-full font-mono text-xs">
              <span className="w-2 h-2 rounded-full bg-pink animate-pulse" /> Currently building: AI Agents & Modern Web Apps
            </div>
          </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 py-20 reveal reveal-delay-2">
        <div className="flex items-end justify-between mb-10 gap-4 flex-wrap">
          ...
          <Link
            to="/work"
            className="font-mono text-sm inline-flex items-center gap-1 hover:gap-3 transition-all"
          >
            View all →
          </Link>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 pb-16 reveal reveal-delay-3">
        <div className="relative bg-electric text-cream brutal-lg rounded-3xl p-10 sm:p-16 overflow-hidden">
          ...
          <h2 className="font-display text-5xl sm:text-7xl max-w-3xl">
            Have a project in mind? <span className="text-lime">Let's talk.</span>
          </h2>
          <p className="mt-4 max-w-lg text-cream/80">
            Web applications, brand sites, integrations, and the occasional ambitious experiment.
            Send a short brief and I'll get back to you.
          </p>
          <div className="mt-8 flex flex-wrap gap-3">
            <a
              href={`mailto:${HAMZA.email}`}
              className="inline-flex items-center gap-2 px-6 py-3 bg-lime text-ink font-mono text-sm rounded-md brutal brutal-hover"
            >
              Email me →
            </a>
            <a
